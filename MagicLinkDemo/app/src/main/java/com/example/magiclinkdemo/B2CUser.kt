...
class B2CUser() {
     ...
     */
     val accounts: MutableList<IAccount> =
    ...
    get(){
        ...
        return getB2CEmailFromAccount(accounts[0])
    }
    /**
    ...
    fun clearSession(multipleAccountPublicClientApplication: IMultipleAccountPublicClientApplication){
        Thread {
            try {
                for (account in accounts) {
                    multipleAccountPublicClientApplication.removeAccount(account)
    ...
    companion object {
        ...
        private fun getB2CEmailFromAccount(account: IAccount): String? {
            return account.claims!!["email"] as String?
        }
    }
}